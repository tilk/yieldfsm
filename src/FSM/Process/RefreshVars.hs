...
refreshVarsCase :: MonadRefresh m => M.Map TH.Name TH.Name -> (TH.Pat, Stmt l) -> m (TH.Pat, Stmt l)
refreshVarsCase m (p, s) = do
    (p', mp) <- refreshPat p
    (p',) <$> refreshVarsStmt (mp `M.union` m) s

refreshVarsStmt :: MonadRefresh m => M.Map TH.Name TH.Name -> Stmt l -> m (Stmt l)
...
refreshVars :: MonadRefresh m => Prog l -> m (Prog l)
refreshVars prog = do
