...
makeLocalVars :: MonadRefresh m => Prog l -> m (Prog l)
...
makeLocalVarsStmt :: (MonadRefresh m, MonadReader LVData m) => Stmt l -> m (Stmt l)
...
makeLocalVarsVStmt :: (MonadRefresh m, MonadReader LVData m) => VStmt -> (TH.Exp -> m (Stmt l)) -> m (Stmt l)
