...
instance IsDesugared l => FreeVars (Stmt l) where
    ...
    freeVars (SYieldT e s) = freeVars e <> freeVars s
...
instance IsDesugared l => Subst (Stmt l) where
    ...
    subst su   (SYieldT e s) = SYieldT (subst su e) (subst su s)
...
isConstructorExpr :: TH.Exp -> Bool
isConstructorExpr (TH.VarE _) = False
isConstructorExpr (TH.ConE _) = True
isConstructorExpr (TH.LitE _) = True
isConstructorExpr (TH.AppE e1 e2) = isConstructorExpr e1 && isConstantExpr e2
isConstructorExpr (TH.AppTypeE e _) = isConstructorExpr e
isConstructorExpr (TH.InfixE me1 e me2) = maybe True isConstantExpr me1 && isConstructorExpr e && maybe True isConstantExpr me2
isConstructorExpr (TH.UInfixE e1 e2 e3) = isConstantExpr e1 && isConstructorExpr e2 && isConstantExpr e3
isConstructorExpr (TH.ParensE e) = isConstructorExpr e
isConstructorExpr (TH.LamE _ _) = False
isConstructorExpr (TH.LamCaseE _) = False
isConstructorExpr (TH.TupE mes) = all (maybe True isConstantExpr) mes
isConstructorExpr (TH.UnboxedTupE mes) = all (maybe True isConstantExpr) mes
isConstructorExpr (TH.UnboxedSumE e _ _) = isConstantExpr e
isConstructorExpr (TH.CondE _ _ _) = False
...
isConstructorExpr (TH.SigE e _) = isConstructorExpr e
isConstructorExpr (TH.RecConE _ res) = all (isConstantExpr . snd) res
isConstructorExpr (TH.RecUpdE e res) = isConstantExpr e && all (isConstantExpr . snd) res
isConstructorExpr (TH.StaticE _) = False
isConstructorExpr (TH.UnboundVarE _) = False
